import React, { useState } from 'react'
import { Link } from 'react-router-dom'
import MenuIcon from '@material-ui/icons/Menu';
import CloseIcon from '@material-ui/icons/Close';
const Header = () => {
    const [isSideBarOpen, setIsSideBarOpen] = useState(false);
    const openSideBar = () => {
        setIsSideBarOpen(true);
    }
    const closeSideBar = () => {
        setIsSideBarOpen(false);
    }
    return (
        // Header Nav
        <div className="header-container">
            <div className="left-side">
                <Link to="/"><img className="header-logo" src="data:image/png;base64," alt="MirrorScore Logo" /></Link>
            </div>
            <div className="menu-icon">
                <button onClick={openSideBar}><MenuIcon className="menu-icon" /></button>
            </div>
            <div className="right-side">
                <ul>
                    <li>
                        <Link to="/">Home</Link>
                    </li>
                    <li>
                        <Link to="/live-class">Live Class</Link>
                    </li>
                    <li>
                        <Link to="/books">NCERT Books</Link>
                    </li>
                    <li>
                        <Link to="/solutions">Solutions</Link>
                    </li>
                    <li>
                        <Link to="/videos">Videos</Link>
                    </li>
                    <li>
                        <Link to="/install"><button className="install-btn">Install Now</button></Link>
                    </li>
                </ul>
            </div>
            <div className={`${isSideBarOpen ? "sidebar open" : "sidebar"}`}>
                <ul>
                    <div className="logo-sidebar">
                        <button onClick={closeSideBar}><CloseIcon className="close-icon-sidebar" /></button>
                        <Link to="/"><img className="header-logo" src="data:image/png;base64," alt="MirrorScore Logo" /></Link>
                    </div>
                    <hr />
                    <li>
                        <Link to="/">Home</Link>
                    </li>
                    <li>
                        <Link to="/live-class">Live Class</Link>
                    </li>
                    <li>
                        <Link to="/books">NCERT Books</Link>
                    </li>
                    <li>
                        <Link to="/solutions">Solutions</Link>
                    </li>
                    <li>
                        <Link to="/videos">Videos</Link>
                    </li>
                </ul>
            </div>
        </div>
    )
}

export default Header